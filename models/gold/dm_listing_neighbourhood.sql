...
chg as (
  select
    b.*,
    lag(active_listings) over (partition by listing_neighbourhood order by year_month) as prev_active,
    lag(total_listings)  over (partition by listing_neighbourhood order by year_month) as prev_total
  from by_suburb b
)
select
  listing_neighbourhood,
  ...
  end as pct_change_total_listings,
